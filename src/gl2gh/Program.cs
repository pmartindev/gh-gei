...
namespace OctoshiftCLI.GitlabToGithub
{
    public static class Program
    {
        ...
        public static async Task Main(string[] args)
        {
            ...
            {
                Logger.LogWarning("Could not retrieve latest gl2gh extension version from github.com, please ensure you are using the latest version by running: gh extension upgrade gl2gh");
                Logger.LogVerbose(ex.ToString());
            }
        ...
        private static async Task LatestVersionCheck(ServiceProvider sp)
        {
            ...
            {
                Logger.LogInformation($"You are running an up-to-date version of the gl2gh extension [v{versionChecker.GetCurrentVersion()}]");
            }
            else
            {
                Logger.LogWarning($"You are running an old version of the gl2gh extension [v{versionChecker.GetCurrentVersion()}]. The latest version is v{await versionChecker.GetLatestVersion()}.");
